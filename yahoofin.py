...
class CryptoDataPull:
    ...
    def get_data(self, symbol, time_interval, last_period):
        ...
        df = self.fetch_data(symbol, start_date.strftime('%Y-%m-%d'), end_date.strftime('%Y-%m-%d'), interval='1d')
...
df_6_months = crypto.get_data('BTC-USD', 'months', 6) # Fetch and store the last 6 months of Bitcoin data

df_6_months_loaded = crypto.load_data('BTC-USD', 'months', 6)

df_6_months_cleaned = crypto.clean_data(df_6_months_loaded)

# Prepare the data for LSTM
X, y, scaler = crypto.prepare_data_for_lstm(df_6_months_cleaned)
